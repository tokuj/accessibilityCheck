...
アクセシビリティチェックAPIのCloud Run + VPC + Cloud NAT構成。固定IPアドレスによる外向き通信を実現し、IP制限のある社内ツールへのアクセスを可能にする。
...
graph TB
    ...
    subgraph GCP["GCP Project: itgproto"]
        subgraph CloudRun["Cloud Run"]
            Service["a11y-check-api<br/>- /api/health<br/>- /api/analyze<br/>- /api/egress-ip"]
        end

        subgraph AR["Artifact Registry"]
            Image["cloud-run-source-deploy<br/>/a11y-check-api:latest"]
        end
    ...
    User -->|HTTPS リクエスト| Service
    Image -.->|デプロイ| Service
    Service -->|VPC Egress: all-traffic| VPCEndpoint
    VPCEndpoint --> Router
    Router --> NAT
    NAT --> StaticIP
...
| Cloud Run | a11y-check-api | asia-northeast1 | Memory: 2Gi, Timeout: 300s |
